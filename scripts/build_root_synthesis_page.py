...
<html lang="en">
  <head>
    ...
    <title>Root Synthesis Essay - Aswath Damodaran Courses Concepts Research</title>
    <style>
      :root {{
        --bg: #f4f1ea;
        --panel: #fffdf8;
...
def build(workspace_root: Path) -> None:
    sections = load_markdown_sections(workspace_root / "analysis/root-synthesis-essay.md")
    hero = """
      <a class="back" href="index.html">Back to root index</a>
      <section class="hero">
        <span class="eyebrow">Root Synthesis</span>
        <h1>What The Active Damodaran Workspace Is Really Showing</h1>
        <p class="lead">
          This essay pulls together the broader societal, cultural, consumer,
          industrial, and pedagogical patterns that now emerge from the active
          multi-course Damodaran workspace.
        </p>
        <div class="meta">
          <span class="chip">Long-form synthesis</span>
          <span class="chip">Cross-course interpretation</span>
    ...
    body = hero + '\n      <div class="stack">\n' + "\n".join(section_html) + "\n      </div>"
    output = workspace_root / "site/root-synthesis-essay.html"
    output.write_text(page(body), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build the root synthesis essay page.")
    parser.add_argument(
        "--workspace-root",
        type=Path,
