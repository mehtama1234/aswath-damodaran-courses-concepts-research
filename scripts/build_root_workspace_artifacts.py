...
def main() -> None:
    ...
    print(f"[1/11] Building root session evidence index for {workspace_root}")
    ...
    print("[2/11] Building root file evidence index")
    ...
    print("[3/11] Resolving normalized concepts to canonical evidence ids")
    ...
    print("[4/11] Rebuilding normalized concepts overview and concept atlas pages")
    build_root_concept_atlas.build(workspace_root)

    print("[5/11] Rebuilding cross-course comparison page")
    build_cross_course_comparison_page.build(workspace_root)

    print("[6/11] Rebuilding valuation audience comparison page")
    build_valuation_audience_comparison_page.build(workspace_root)

    print("[7/11] Rebuilding root synthesis essay page")
    build_root_synthesis_page.build(workspace_root)

    print("[8/11] Rebuilding sector and company writeup framework page")
    build_sector_company_framework_page.build(workspace_root)

    print("[9/11] Rebuilding root applied analysis pages")
    build_root_applied_analysis_pages.build(workspace_root)

    print("[10/11] Rebuilding applied analysis index page")
    build_applied_analysis_index_page.build(workspace_root)

    print("[11/11] Rebuilding applied evidence page")
