...
<html lang="en">
  <head>
    ...
    <title>Sector And Company Writeup Framework - Aswath Damodaran Courses Concepts Research</title>
    <style>
      :root {{
        --bg: #f4f1ea;
        --panel: #fffdf8;
...
def build(workspace_root: Path) -> None:
    sections = load_markdown_sections(workspace_root / "analysis/sector-and-company-writeup-framework.md")
    hero = """
      <a class="back" href="index.html">Back to root index</a>
      <section class="hero">
        <span class="eyebrow">Writeup Framework</span>
        <h1>Sector And Company Writeup Framework</h1>
        <p class="lead">
          A root-level guide for turning the Damodaran concept, comparison, and
          synthesis layers into detailed but easy-to-understand sector and
          company analysis.
        </p>
        <div class="meta">
          <span class="chip">Reusable writing scaffold</span>
          <span class="chip">Sector and company analysis</span>
    ...
    body = hero + '\n      <div class="stack">\n' + "\n".join(section_html) + "\n      </div>"
    output = workspace_root / "site/sector-and-company-writeup-framework.html"
    output.write_text(page(body), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build the sector and company writeup framework page.")
    parser.add_argument(
        "--workspace-root",
        type=Path,
