...
<html lang="en">
  <head>
    ...
    <title>Root Themes Atlas - Aswath Damodaran Courses Concepts Research</title>
    <style>
      :root {{
        --bg: #f4f1ea;
        --panel: #fffdf8;
...
def build(workspace_root: Path) -> None:
    sections = load_markdown_sections(workspace_root / "analysis/root-themes-atlas.md")
    hero = """
      <a class="back" href="index.html">Back to root index</a>
      <section class="hero">
        <span class="eyebrow">Themes Atlas</span>
        <h1>Societal, Cultural, Consumer, And Industrial Themes Atlas</h1>
        <p class="lead">
          A root-level atlas of the broader themes emerging across the active
          Damodaran workspace, meant to bridge course interpretation and later
          sector, company, and curriculum work.
        </p>
        <div class="meta">
          <span class="chip">Theme clusters</span>
          <span class="chip">Cross-course synthesis</span>
    ...
    body = hero + '\n      <div class="stack">\n' + "\n".join(section_html) + "\n      </div>"
    output = workspace_root / "site/root-themes-atlas.html"
    output.write_text(page(body), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build the root themes atlas page.")
    parser.add_argument(
        "--workspace-root",
        type=Path,
